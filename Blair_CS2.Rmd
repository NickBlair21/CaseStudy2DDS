...
##Thus, we do not observe correlation for these variables. Instead, we can observe the percentage of Attrition experienced in each category.
...
# 
# knnTraining = noIDnumericData %>% select(Attrition, Age, JobInvolvement, JobLevel, JobSatisfaction, MonthlyIncome, StockOptionLevel, TotalWorkingYears, WorkLifeBalance, YearsAtCompany, YearsInCurrentRole,YearsWithCurrManager)
...
#Normalize data
# subset.knnTraining <- as.data.frame(lapply(knnTraining[,2:16], normalize))
subset.knnTraining <- knnTraining[,2:19]
...
knn.24 <- knn(train=train, test = test, cl = train.attrition, k=24)
knn.25 <- knn(train=train, test = test, cl = train.attrition, k=25)


acc.24 <- 100 * sum(test.attrition == knn.24)/nrow(test.attrition)
acc.25 <- 100 * sum(test.attrition == knn.25)/nrow(test.attrition)

acc.24
acc.25

confusionMatrix(table(knn.24, test.attrition))
confusionMatrix(table(knn.25, test.attrition))
...
rfTraining = fullData
#Convert Attrition factor levels from string to numeric (1 = "No", 2 = "Yes")
# rfTraining$Attrition <- as.numeric(numericData$Attrition)

subset.rfTraining = rfTraining

# #Normalize data
# # subset.knnTraining <- as.data.frame(lapply(knnTraining[,2:16], normalize))
# subset.knnTraining <- knnTraining[,8:16]
...
# 
# randomForest(Attrition ~ JobInvolvement + TotalWorkingYears + JobLevel + YearsInCurrentRole + MonthlyIncome + Age + StockOptionLevel + YearsWithCurrManager
#                + YearsAtCompany + JobSatisfaction + WorkLifeBalance + JobRole, data = train, method = 'rf', trControl = trainControl(method = 'cv', number=5))
...
predictLM = predict.lm(LMmodel, newdata = numericTestingData)
